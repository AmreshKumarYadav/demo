...
public class SeleniumSupportFunctions extends BaseClass {
	private static int maxLoadTime = 50;
	private static final int BUFFER_SIZE = 4096;
	private static WebElement objectElement;
	...
	public static WebDriver waitForPageToLoad() {
		try {
			new WebDriverWait(getDriver(), explicitWaitTime)
	...
	public static void waitForText(String regExText, int secondsToWait) {
		...
		while (true) {
			...
			if (now.getTime().getTime() < future.getTime().getTime()) {
				Matcher m = p.matcher(getDriver().getPageSource());
				if (m.find()) {
					return;
				} else {
					try {
	...
	public static boolean switchToWindow(String title) {
		...
		for (String Child_Window : getDriver().getWindowHandles()) {
			...
			if (getDriver().getTitle().toString().isEmpty()) {
	...
	public static boolean isTextPresentOnPage(String text, int waitSeconds) {
		try {
			...
			return getDriver().findElement(By.xpath("//*[text()='" + text + "']")).isDisplayed();
		} catch (Throwable t) {
			return false;
		}
	}
